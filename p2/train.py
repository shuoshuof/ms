...
class mymodel(object):
    ...
    def get_model(self):
        ...
        x1 = keras.layers.LSTM(units=64, return_sequences=True)(x1)
        x1 = keras.layers.Dropout(0.2)(x1)
        x1= keras.layers.LSTM(units=32,return_sequences=True)(x1)
        x1 = keras.layers.Dropout(0.2)(x1)
        x1= keras.layers.LSTM(units=8,return_sequences=False)(x1)
        x1 = keras.layers.Dropout(0.2)(x1)

        inputs_2 = keras.Input(shape=self.input_shape_2)
        x2 = keras.layers.Dense(8,activation='tanh')(inputs_2)

        x = keras.layers.concatenate([x1, x2])
        x = keras.layers.Dense(32,activation='tanh')(x)
        x = keras.layers.Dropout(0.2)(x)
        outputs = keras.layers.Dense(self.class_num,activation='softmax')(x)
        model = keras.Model([inputs_1,inputs_2],outputs)
        model.compile(loss=keras.losses.MeanAbsoluteError(),optimizer=keras.optimizers.Adam(1e-3))
        model.summary()
        return model
